...
SAML.prototype.decryptXml = function(token, callback) {
  var encryptedData = token.getElementsByTagNameNS('http://www.w3.org/2001/04/xmlenc#', 'EncryptedData')[0];
  if (!encryptedData) {
    callback(new Error('EncryptedData not found.'));
  }

  if (!this.options.decryptionKey) {
    callback(new Error('Assertion is encrypted. Please set options.decryptionKey with your decryption private key.'));
  }

  xmlenc.decrypt(encryptedData.toString(), { key: this.options.decryptionKey, autopadding: this.options.autopadding }, callback);
}
...
SAML.prototype.validateSamlAssertion = function (samlAssertion, callback) {
  ...
  if(samlAssertion.localName == 'EncryptedAssertion' && samlAssertion.namespaceURI == saml2Namespace) {
    encryptedToken = samlAssertion;
  } else {
    encryptedToken = samlAssertion.getElementsByTagNameNS(saml2Namespace, 'EncryptedAssertion')[0];
  }

  if (encryptedToken) {
    self.decryptXml(encryptedToken, function(error, data) {
      if(error) callback(error, null);
      self.validateSamlSignature(data, callback);
    });
  } else {
    self.validateSamlSignature(samlAssertion, callback);
  }
};

SAML.prototype.validateSamlSignature = function(samlAssertion, callback) {
  var self = this;
